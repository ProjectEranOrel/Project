...
public class SequenceAlignment {
	...
	public String seq1Aligned, seq2Aligned;
	...
	public void calcOptimalAlignment(Sequence seq1, Sequence seq2){
		seq1.setDNA(seq1.getDNA().toLowerCase());
		seq2.setDNA(seq2.getDNA().toLowerCase());

		seq1.setDNA(seq1.getDNA().trim());		//trim any whitespace
		seq2.setDNA(seq2.getDNA().trim());

		seq1.setDNA(seq1.getDNA().replaceAll(" ", ""));	//Remove spaces
		seq2.setDNA(seq2.getDNA().replaceAll(" ", ""));

		seq1.setDNA(" " + seq1.getDNA());		//prepend a space @ the start. Allows for easier calls to mismatchPenalty() & array boundaryies for size of memoTalbe to be "<" instead of "<="
		seq2.setDNA(" " + seq2.getDNA());

		//Initialize 2D arrays for memoization
		memoTable = new int[seq1.getDNA().length()][seq2.getDNA().length()];
		predecessorIndexes = new int[seq1.getDNA().length()][seq2.getDNA().length()][2];

		//Array bounds are < seq1.getDNA().length() (not <= ) since both sequences have a blank space @ the start
		//Fill 0th column
		for(int i=0; i<seq1.getDNA().length(); i++){
			memoTable[i][0] = i * GAP_PENALTY;		// base case: j = 0
			predecessorIndexes[i][0][0] = i-1;
			predecessorIndexes[i][0][1] = 0;
		}
		//Fill 0th row
		for(int j=0; j<seq2.getDNA().length(); j++){
			memoTable[0][j] = j * GAP_PENALTY;		// base case: i = 0
			predecessorIndexes[0][j][0] = 0;
			predecessorIndexes[0][j][1] = j-1;
		}
		...
		//Fill rest of memo table
		for(int j=1; j<seq2.getDNA().length(); j++){
			for(int i=1; i<seq1.getDNA().length(); i++){
				int bothAligned = mismatchPenalty(seq1.getDNA().charAt(i), seq2.getDNA().charAt(j)) + memoTable[i-1][j-1];	//case1: seq1[i] & seq2[j] aligned with each other
				int seq1WithGap = GAP_PENALTY+memoTable[i-1][j];		//case2: seq1 with gap
				int seq2WithGap = GAP_PENALTY+memoTable[i][j-1];		//case3: seq2 with gap
				//Calculate the min of 3 values & store predecessors
				if(bothAligned<=seq1WithGap && bothAligned<=seq2WithGap){		//case1 smallest
	...
	//	System.out.println("\n" + memoTable[seq1.getDNA().length()-1][seq2.getDNA().length()-1] + "\t is the Minimum penalty for aligning \""+seq1.getDNA().substring(1, seq1.getDNA().length()) +"\" & \""+seq2.getDNA().substring(1, seq2.getDNA().length()) +"\"");
		findAlignment(seq1.getDNA(), seq2.getDNA());
	}
